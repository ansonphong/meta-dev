...
4. Scaffold `_runbook-<today>.md` in the feature dir from `references/runbook-template.md`: write the
   frontmatter (`type: runbook`, ordered `members:`, `predecessor:` if chaining), the narrative
   sections (HIGHER CONCEPT / PACKAGE / DEPENDENCY ORDER / GATES / LIVE STATUS), and the dashboard
   sentinels.
...
- `done` — mark a member's status `done` (its frontmatter is the truth; the dashboard follows).
