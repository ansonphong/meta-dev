---
name: auto-execute
argument-hint: <any task, prompt, plan, or meta-dev op> [--deep|--grok|--codex|--sonnet|--glm] [--flash] [--effort <level>] [--repo <name>] [--readonly] [--max-turns <n>] [--autonomous]  # --repo names from .claude/meta-dev-repos.json
...
- `--deep` / `--glm` / `--sonnet` / `--codex` — force a backend, skip routing (still chunk + review). `--sonnet` pins each chunk to a separate headless Anthropic Sonnet 5 worker (`claude-headless-exec --backend sonnet`, `claude-sonnet-5`) — Anthropic-grade judgment in its own process, so the chunk's context churn never lands in the conductor's window. `--codex` is a first-class executor **and** the cross-family review lens (see Core Bias) — route it spark-first and dispatch through `scripts/codex-headless-exec` (`--sandbox workspace-write` when it edits, `--sandbox read-only` when it only reports back). `--deep` defaults to `deepseek-v4-pro` (V4-Pro-0813). Pass `--flash` on a mechanical chunk to force Flash.
