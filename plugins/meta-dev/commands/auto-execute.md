...
4. **Round-trip review** — when a chunk returns, **read its distilled result AND the actual change** (`git diff`/`git status` of what it touched, or re-run its verify hook). You (Opus) are the reviewer — this is the quality gate, not the worker's self-report.
5. **Pass / fail:**
   - **Pass** → integrate, mark the chunk done, go to the next.
   - **Fail** → fix inline if trivial, else **re-dispatch — escalating DeepSeek→GLM** (a chunk DeepSeek fumbled is exactly an escalation signal). Don't loop the same backend on the same failure twice.
...
4. **Code-review the phase** when the worker returns — read the worker's distilled result **and** the real phase diff (`git diff` over the phase's commits / re-run the phase verify). This is your quality gate, per the Round-trip review step.
5. **Advance.** Phase clean → move to phase N+1 and repeat. Phase fails review → fix inline if trivial, else re-dispatch escalating DeepSeek→GLM. Respect dependencies: never start phase N+1 if it depends on a phase that hasn't landed green.
