...
3. **Service & runtime probes** (backend modes) — import smoke, `db.engine.connect()`, Redis ping, queue import + Redis-degradation heuristic. `required:false` failures → WARN, not BLOCKED.
