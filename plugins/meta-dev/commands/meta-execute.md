...
argument-hint: <plan-path> [--inline] [--strict] [--review each|phase|end|auto] [--deep] [--grok] [--codex] [--sonnet] [--glm] [--effort <level>] [--deploy] [--pause-before=<task-id>]
...
| `--deep` | Per-task headless DeepSeek worker + phase-gated review-agent (`deepseek-v4-pro` default, V4-Pro-0813; pass `--flash` on mechanical tasks; fix-escalation goes one rung up `meta_dev.ladder.pool`) |
