...
argument-hint: <plan-path> [--inline] [--strict] [--review each|phase|end|auto] [--budget auto|low|medium|high] [--deep] [--grok] [--codex] [--sonnet] [--glm] [--agy] [--effort <level>] [--deploy] [--pause-before=<task-id>]
...
### Worker tier (`--deep`/`--grok`/`--codex`/`--sonnet`/`--glm`/`--agy`)
...
**Bare invocation (no tier flag) = host-native subagents, never this thread** — steps 1–8 below. Use the Host dispatch table. No external process is spawned unless a tier flag or the host's pooled-worker overlay says so. `--deep`/`--grok`/`--codex`/`--sonnet`/`--glm`/`--agy` are **explicit opt-ins** that force a specific *foreign* backend — none of them is the default, and none of them is an excuse to implement here. `--agy` is parked: only when Phong named Antigravity this turn. `--inline` is the only "this thread types the code" flag.
...
| `--agy` | Per-task headless **Antigravity** worker (`scripts/agy-headless-exec`). **Parked — only when Phong named it this turn.** Default **Gemini 3.7 Flash** (1M context, native image/video/audio, Google Search grounding). `--opus` on that runner is **Claude Opus 4.6 Thinking on Google quota**, not Claude Code / not Opus 5. Inner host-native subagents stay Grok/`Agent`/spark — `--agy` replaces the per-checkbox worker, it does not nest under them. |
