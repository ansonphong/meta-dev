...
**Safe parallel waves.** Default is to co-dispatch every READY task whose declared file set is disjoint from in-flight workers, up to 8 in-flight from this run. Serialize when files overlap, file set is unknown, `--review each`, `--inline`, or `--glm`. `--strict` serializes verify, not disjoint dispatch. The checkbox flip is per completed task; it does not force the rest of the run serial.

## Conductor law — this thread does not implement

The `/meta-execute` conductor does **not** implement plan tasks. Each verifiable checkbox is a fresh host-native subagent (Grok Build → `spawn_subagent` with **grok-4.5** collect / **grok-4.6** real work; Claude Code → pooled Grok/Codex, not a same-family Agent; Codex → Spark/Luna mechanical, Terra ordinary, Sol hard). **Farm pieces** so the conductor context stays verdicts. Pick the cheapest rung (`references/work-ladder.md`). Shape each brief for that backend (`references/execute-briefs.md`) — collect is `TASK` + `RETURN`, not a novel. The worker commits its own edits. The conductor reads a one-line result and flips `task-done`. `--inline` is the only exception, and only when the user passed it. Missing a worker primitive is a bug in the host table, not permission to type the plan on this thread. When unsure, spawn. Stay native only for **this thread's** slash palette, vision, a true one-liner, permission/stage gates, and integrating that one-line return. Do not dispatch DeepSeek unless Phong named it. Headless Grok/Codex get a direct task, not a Claude slash — `references/work-ladder.md` → *Who has meta-dev*.
