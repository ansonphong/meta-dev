...
**Invoked by `/meta-dev`** at the start of any autopilot/walk run (explicit cruise/autonomous intent, not incidental keywords or Accept-Edits mode). Mandatory in autopilot/walk; recommended-but-optional in interactive mode.

## Procedure

### 1. Create once, up front (before Stage 1)

Use the available host-native tracker (Claude `TaskCreate` or equivalent) — one entry per waterfall stage, named for what it does. Chain dependencies (each stage depends on the prior):
...
Stage 6 — Review (one native covering verdict)
```

If no native tracker is available, report concise stage progress and retain planctl state; do not invent TaskCreate or block authorized work. For multi-item runs, create one stage list per subject and prefix entries with the subject so concurrent pipelines stay legible.

### 2. Update as state changes (never batched at the end)

- Stage starts → `TaskUpdate` to `in_progress`.
- Exit criteria met (the per-stage exit-criteria table in the plugin's `dev-modes` reference) → `completed`; persist related artifact changes at a safe seam.
