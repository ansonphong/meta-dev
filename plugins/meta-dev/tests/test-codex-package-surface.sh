...
assert len(canonical_commands) == 44, f"expected 44 canonical commands, found {len(canonical_commands)}"
helper_only = {"plan", "execute", "harden", "review", "diagnose", "ops"}
assert native_dirs == canonical_commands | helper_only | {"command-router", "sniff-test"}
assert len(native_dirs) == 52, f"expected 44 commands, 6 helpers, router, and sniff-test; found {len(native_dirs)}"
sniff_skill = (skills_root / "sniff-test/SKILL.md").read_text(encoding="utf-8")
assert "../../workflow-skills/sniff-test/SKILL.md" in sniff_skill
assert "Do not edit the target" in sniff_skill
...
assert route_schema["properties"]["tier"]["enum"] == ["spark", "luna", "terra", "sol", "astra"]
...
assert jsonschema.Draft7Validator(route_schema).is_valid({"tier": "luna", "effort": "low"})
