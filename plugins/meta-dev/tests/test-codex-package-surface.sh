...
assert len(canonical_commands) == 41, f"expected 41 canonical commands, found {len(canonical_commands)}"
helper_only = {"plan", "execute", "harden", "review", "diagnose", "ops"}
assert native_dirs == canonical_commands | helper_only | {"command-router"}
assert len(native_dirs) == 48, f"expected 41 commands, 6 helpers, and router; found {len(native_dirs)}"
