...
helper_targets = {
    ...
    for name in ("plan", "harden", "review", "runbook", "diagnose", "ops")
}
helper_text = []
...
assert len(canonical_commands) == 40, f"expected 40 canonical commands, found {len(canonical_commands)}"
helper_only = {"plan", "harden", "review", "diagnose", "ops"}
assert native_dirs == canonical_commands | helper_only | {"command-router"}
assert len(native_dirs) == 46, f"expected 40 commands, 5 helpers, and router; found {len(native_dirs)}"
...
assert discovery_bytes < 5000, f"Codex discovery metadata is {discovery_bytes} bytes before cache prefix"
...
print(
    ...
    f"67 routes, {discovery_bytes} discovery bytes)"
)
PY
